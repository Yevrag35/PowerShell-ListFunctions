...
{
    public abstract class ScriptBlockFilterOld
    {
        private protected ScriptBlockFilterOld()
        {
        }

        protected abstract object MakePredicate();

        public static ScriptBlockFilterOld Create(ScriptBlock scriptBlock, Type genericType, IEnumerable<PSVariable>? additionalVariables)
        {
            Type filterType = typeof(ScriptBlockFilterOld<>);
            ...
            return Activator.CreateInstance(genFilter,
                new object[] { scriptBlock, additionalVariables! })
                    as ScriptBlockFilterOld
                    ?? throw new InvalidOperationException("Unable to create generic script block filter instance.");
        }
        public static object ToPredicate(ScriptBlockFilterOld genericScriptBlock)
        {
            return genericScriptBlock.MakePredicate();
        }
    }

    public sealed class ScriptBlockFilterOld<T> : ScriptBlockFilterOld
    {
        ...
        public ScriptBlockFilterOld(ScriptBlock scriptBlock)
            : this(scriptBlock, null)
        {
        }
        public ScriptBlockFilterOld(ScriptBlock scriptBlock, IEnumerable<PSVariable>? additionalVariables)
        {
        ...
        public static implicit operator Predicate<T>(ScriptBlockFilterOld<T> filter)
        {
